<template>
  <div>
      <div v-for="(post) in posts" :key="post.guid" :class="colClass">
        ...
        <div class="excerpt" v-html="formatPostDescription(post.description)">
        </div>
        <div class="read-more"><a :href="post.link" @click.prevent="showPost($event, post.link)" target="_blank">Read more</a></div>
      </div>
    </div>
</template>
